...
class CategoryModel {
  ...
  static async getById(id) {
    const [rows] = await db.query("SELECT * FROM category WHERE cate_id= ?", [
      id,
    ]);
    return rows[0];
  }
  ...
  static async delete(id) {
    const [result] = await db.query("DELETE FROM category WHERE cate_id= ?", [
      id,
    ]);
  ...
  static async update(id, data) {
    const [result] = await db.query("UPDATE category SET ? WHERE cate_id= ?", [
      data,
      id,
    ]);
